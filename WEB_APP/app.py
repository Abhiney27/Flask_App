...
def register():
    if request.method == 'POST':
        ...
        existing_mail = client[db][collection].find_one({"email":email})
        ...
        if isValid == True:
            ...
            client[db][collection].insert_one({"name" : name , "mobile" : mobile , "email" : email , "password": hashpass,"SignUptime" : SignUptime})
    ...
    if request.method == "POST":
        ...
        if login_email:
            if bcrypt.hashpw(request.form['password'].encode('utf-8') , login_email['password']) == login_email['password']:
                ...
                client[db][collection].update_one({"email" : email} ,{ "$set":{"lastLoginTime": lastLoginTime}})
...
def update_passwd():
    if request.method == "POST":
        ...
        client[db][collection].update_one({"email" : email} ,{ "$set":{"password": hashpass},"$currentDate":{"lastModified":True}
})
        return render_template("login.html")
